...
$(document).ready(function($) {
    $.getJSON('assets/timeline.json', function(data) {
        ...
        $.each(data, function(key, val) {
            let selected = key === 0 ? 'class="selected"' : ''
            LinkOutput += '<li><a href="#0" data-date="' + val.linkDate + '" ' + selected + '>' + val.linkTitle + '</a></li>'

            DataOutput += '<li ' + selected + ' data-date="' + val.linkDate + '">'
            DataOutput += '<h2>' + val.mainTitle + '</h2>'
            DataOutput += '<em>' + val.mainDate + '</em>'
            DataOutput += '<p>' + val.message + '</p></li>'
        });
        $('.events-content ol').html(DataOutput);
        $('.events ol').html(LinkOutput);
    }).done(function() {
